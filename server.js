...
app.get('/todos',function(req,res){
	...
	var filteredItems = todos;
		...
		if(queryParams.hasOwnProperty('completed') && queryParams.completed === 'true'){
			filteredItems = _.where(todos, {completed: true});
		}
...
app.put('/todos/:id', function (req, res){
	...
	var validAttributes = {};
		...
		if(!matchedItem){
			res.send(404).json({"error": "Not found with that id "});
		}
		...
		//	matchedItem.completed = validAttributes.completed;
			...
			return res.status(400).json({"error": "Something wrong with completed"});
		}
		...
		//	matchedItem.description = validAttributes.description;
			...
			return res.status(400).json({"error": "Something wrong with description"});
		}
